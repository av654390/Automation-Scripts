...
for i in system_ids:
    try:
        ...
        if i.lower() == "bip":
            # This block will run only for 'biv' system
            val1 = db02.db02(nos=nos, session=session, stamp=stamp, path=path, sid=i)
            nos += 1
            average_cpu = val1.get("cpumemory")
            replication_status = val1.get("replicationstatus")
            print(f"AvgCPU_DB: {average_cpu}")
            #print("cpumemory ===", average_cpu)
            ws.cell(row=row, column=c, value=average_cpu)
            ...
            #print("replicationstatus ===", replication_status)
            ws.cell(row=row, column=c+7, value=replication_status)
            if replication_status == "Green":
                ws.cell(row=row, column=c+7).fill = green_fill
                all_status.append("Green")
            elif replication_status == "Amber":
                ws.cell(row=row, column=c+7).fill = amber_fill
                all_status.append("Amber")
            else:
...
try:
    ...
    To = ["akoju.x.sharanya@gsk.com"]#,"nikhil.x.chalikwar@gsk.com","erry.8.lavakumar@gsk.com"
    Cc = ["akoju.x.sharanya@gsk.com"]
    #To = ["vxrepbasis@gsk.com","ITTCSESAT@gsk.com","jay.x.menon@gsk.com","sumeet.x.paprikar@gsk.com"]
    #Cc = ["bunmi.x.jolaoso@gsk.com","krishnamurthy.x.k@gsk.com"]
    Bcc = ["akoju.x.sharanya@gsk.com"]#,"nikhil.x.chalikwar@gsk.com","erry.8.lavakumar@gsk.com"
    sub = f"[{overall_status}] : AutoSAP Wave 1 hourly system monitoring {stamp} CET"
