# ****************************************************************************
#  Program Description : Script Performs sending mail activity               *
#  Program Name:  AutosapABAP Dumps\autosapABAPdumps.py                      *
#          Date:  30/04/2025                                                 *
#       version:  1.0.0                                                      *
...
def send_mail(send_from, send_to,send_bcc,send_cc,subject, text, files=None,html=None): # 
    ...
    for f in os.listdir(files) or []:
        ...
        if fi < 20 and (f.lower().endswith(".html") or f.lower().endswith(".png")):
...
def error_mail(lid,err,sid):
    try:
        ...
        msg['Cc'] = "akoju.x.sharanya@gsk.com"
        msg['Date'] = formatdate(localtime=True)
        msg['Subject'] = f"Logon Error occured in EWA Dumps Report {formatdate(localtime=True)}"
                ...
                <tr>
                    <td style="padding: 0; text-align: center;">
                        <div style="background: #F36633; width: 80px; height: 150px; margin: auto; display: flex; align-items: center; justify-content: center;">
                            <strong><span class="content" style="font-size: 24px;">AutoSAP EWA Dumps Monitoring</span></strong>
                        </div>
                    </td>
                </tr>
