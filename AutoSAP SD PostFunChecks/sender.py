...
#  Program Name:  AutoSAP SD PostFunChecks\sender.py                                     *
#          Date:  15/05/2025                                                 *
#       version:  1.0.0                                                      *
...
def send_mail(send_from, send_to, send_cc, send_bcc, subject, text, files=None,html=None,errors=None):
    ...
    for f in os.listdir(files) or []:
        ...
        if errors:
            if fi < 20 and f.lower().endswith(".docx") and "error" in f.lower():    
        ...
        elif fi < 20 and f.lower().endswith(".docx"):    
            with open(files+"\\"+f, "rb") as fil:
                part = MIMEApplication(
                    fil.read(),
                    Name=basename(f)
...
def error_mail(lid, err, sid):
    try:
        ...
        msg['To'] = "akoju.x.sharanya@gsk.com,erry.8.lavakumar@gsk.com,shiva.x.prasad@gsk.com,akash.2.verma@gsk.com"
        msg['Cc'] = "akoju.x.sharanya@gsk.com,erry.8.lavakumar@gsk.com"
        msg['Date'] = formatdate(localtime=True)
        msg['Subject'] = f"Logon Error occured in {sid}- AutoSAP SD {sid} Post Functional Checks {stamp} {formatdate(localtime=True)}"
                ...
                <tr>
                    <td style="padding: 0; text-align: center;">
                        <div style="background: #F36633; width: 80px; height: 150px; margin: auto; display: flex; align-items: center; justify-content: center;">
                            <strong><span class="content" style="font-size: 24px;">AutoSAP SD {sid} Post Functional Checks {stamp}</span></strong>
                        </div>
                    </td>
                </tr>
