...
def genxl(final_values=0,run_time=0,path=0,stamp=0):
    ...
    #####SE38#####
    #CPU Utilization%
    cpu=final_values['st06_cpu_dict']['AvgCPU']['value']
    ws.cell(row=6,column=3).value=str(cpu)
    fill_color(status=final_values['st06_cpu_dict']['AvgCPU']['status'],irow=6)
    #Logins
    logins=final_values['se38_dict']['logins']['value']
    ws.cell(row=17,column=3).value=str(logins)
    fill_color(status=final_values['se38_dict']['logins']['status'],irow=17)
    #Sessions
    sessions=final_values['se38_dict']['sessions']['value']
    ws.cell(row=20,column=3).value=str(sessions)
    fill_color(status=final_values['se38_dict']['sessions']['status'],irow=20)
    #WPS Priv Mode
    wps=final_values['se38_dict']['wps']['value']
    ws.cell(row=26,column=3).value=str(wps)
    fill_color(status=final_values['se38_dict']['wps']['status'],irow=26)
    ...
    ######SM37######
    #Active Jobs
    ws.cell(row=12,column=3).value=str(final_values['sm37_dict']['value'])
    fill_color(status=final_values['sm37_dict']['status'],irow=12)

    ######ST07######
    #Active Users
    ws.cell(row=9,column=3).value=final_values['st07_dict']['act']['value']
    fill_color(status=final_values['st07_dict']['act']['status'],irow=9)
    #Work Process Occupied
    ws.cell(row=14,column=3).value=final_values['st07_dict']['wpo']['value']
    fill_color(status=final_values['st07_dict']['wpo']['status'],irow=14)

    ######ST22######
    # Time Out Dumps
    ws.cell(row=23,column=3).value=final_values['st22_dict']['value']  
    fill_color(status=final_values['st22_dict']['status'],irow=23)
    ...
    ######Seal Server(SM59)####
    ws.cell(row=32,column=3).value=final_values['sm59_dict']['value']
    fill_color(status=final_values['sm59_dict']['status'],irow=32)
